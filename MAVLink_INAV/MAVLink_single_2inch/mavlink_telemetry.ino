...
// Configuration globals from config tab
extern const float CONFIG_AHI_PITCH_OFFSET_DEG;
extern const float CONFIG_MAVLINK_ROLL_SIGN;
extern const uint8_t CONFIG_BATTERY_CELL_COUNT;
...
void decodeMavlinkAttitude()
{
  ...
  rawRoll =
    attitude.roll * CONFIG_MAVLINK_ROLL_SIGN;
  ...
  // ATTITUDE packets update targets only. The display task advances the
  // visible values every frame, avoiding packet-rate stepping.
