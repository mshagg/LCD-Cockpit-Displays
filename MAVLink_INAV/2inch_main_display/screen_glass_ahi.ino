// ----------------------------------------------------
// Glass-style full-screen AHI
// ----------------------------------------------------
//
// Inspired by a Bendix/King style electronic attitude display,
// but without airspeed, altitude, heading, or non-functional side tapes.
//
// Main features retained:
// - black panel / display frame
// - blue/brown attitude field
// - top roll scale and pointer inside the attitude box
// - pitch ladder
// - fixed aircraft cue
// ----------------------------------------------------
...
void drawGlassAhiScreen(Arduino_Canvas *display, float roll, float pitch)
{
  ...
  // Outer electronic-display frame
  const int bezelX = 18;
  const int bezelY = 24;
  const int bezelW = 204;
  const int bezelH = 204;
  ...
  // Roll scale: 10% larger than before.
  // Top of the arc touches the top edge of the attitude field.
  const int rollScaleRadius = 46;
  const int rollScaleCy = attY + rollScaleRadius;
  drawGlassAhiRollScale(display, cx, rollScaleCy, rollScaleRadius, whiteColor);
...
void drawGlassAhiPitchLadder(
  ...
  for (int deg = -30; deg <= 30; deg += 5) {
    ...
    // Around 10% wider on each side than the earlier version.
    int halfLen = (deg % 10 == 0) ? 31 : 15;
    ...
    if (deg % 10 == 0) {
      ...
      // Labels moved outward to match the wider ladder.
...
void drawGlassAhiRollScale(
  ...
  // Top pointer triangle, kept inside the attitude box
  ...
  // Small reference triangle beneath
  display->fillTriangle(
    cx,      cy - radius + 25,
    cx - 7,  cy - radius + 37,
    cx + 7,  cy - radius + 37,
...
void drawGlassAhiAircraftCue(
  ...
  // ----------------------------------------------------
  // Magenta flight-director style command bars
  // ----------------------------------------------------
  //
  // These sit behind the fixed yellow aircraft symbol.
  ...
  // ----------------------------------------------------
  //
  // Short yellow wing bars, black pointed ends,
  // and central yellow nose/caret reference.
