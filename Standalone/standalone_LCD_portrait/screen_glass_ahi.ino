// ----------------------------------------------------
// Glass-style portrait AHI
// ----------------------------------------------------
//
// Built-in Waveshare ESP32-S3-LCD-1.69 display only.
//
// Portrait layout:
// - canvas size: 240 x 280
// - enlarged glass attitude field
// - equal top/bottom blank margins
// - fixed roll scale arc
// - fixed zero-bank reference triangle
// - moving yellow bank pointer
// - fixed aircraft cue
// ----------------------------------------------------
...
void drawGlassAhiScreen(Arduino_Canvas *display, float roll, float pitch)
{
  ...
  //
  // Screen: 240 x 280
  // Top blank margin    = 24 px
  // Bottom blank margin = 24 px
  const int bezelX = 18;
  const int bezelY = 24;
  const int bezelW = 204;
  const int bezelH = 232;
...
void drawGlassAhiAircraftCue(
  ...
  display->fillTriangle(cx - 57, cy + 8, cx - 8,  cy - 8, cx - 15, cy + 17, magentaColor);
  display->fillTriangle(cx + 57, cy + 8, cx + 8,  cy - 8, cx + 15, cy + 17, magentaColor);
  ...
  display->fillTriangle(cx - 72, wingY - 4, cx - 54, wingY - 4, cx - 54, wingY + 4, blackColor);
  display->fillTriangle(cx + 72, wingY - 4, cx + 54, wingY - 4, cx + 54, wingY + 4, blackColor);

  display->fillRoundRect(cx - 67, wingY - wingH / 2, 44, wingH, 2, yellowColor);
  display->fillRoundRect(cx + 23, wingY - wingH / 2, 44, wingH, 2, yellowColor);

  display->drawFastHLine(cx - 67, wingY + 4, 44, blackColor);
  display->drawFastHLine(cx + 23, wingY + 4, 44, blackColor);

  display->fillTriangle(cx, wingY - 13, cx - 16, wingY + 7, cx + 16, wingY + 7, yellowColor);
  display->fillTriangle(cx, wingY - 6, cx - 8, wingY + 5, cx + 8, wingY + 5, blackColor);
