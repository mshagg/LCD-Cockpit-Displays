// ----------------------------------------------------
// Full-screen futuristic Tron-style landscape AHI
// ----------------------------------------------------
//
...
void drawTronFullCornerFrame(
  ...
  // Top left
  display->drawLine(8, 8, 44, 8, cyanBright);
  display->drawLine(8, 8, 8, 34, cyanBright);
  display->drawLine(8, 20, 28, 20, cyanDim);
  display->drawLine(20, 8, 20, 26, cyanDim);

  // Top right
  display->drawLine(w - 9, 8, w - 45, 8, cyanBright);
  display->drawLine(w - 9, 8, w - 9, 34, cyanBright);
  display->drawLine(w - 9, 20, w - 29, 20, cyanDim);
  display->drawLine(w - 21, 8, w - 21, 26, cyanDim);

  // Bottom left
  display->drawLine(8, h - 9, 44, h - 9, cyanBright);
  display->drawLine(8, h - 9, 8, h - 35, cyanBright);
  display->drawLine(8, h - 21, 28, h - 21, cyanDim);
  display->drawLine(20, h - 9, 20, h - 27, cyanDim);

  // Bottom right
  display->drawLine(w - 9, h - 9, w - 45, h - 9, cyanBright);
  display->drawLine(w - 9, h - 9, w - 9, h - 35, cyanBright);
  display->drawLine(w - 9, h - 21, w - 29, h - 21, cyanDim);
  display->drawLine(w - 21, h - 9, w - 21, h - 27, cyanDim);

  // Side rails
  display->drawLine(15, 54, 15, h - 54, cyanVeryDim);
  display->drawLine(20, 68, 20, h - 68, cyanDim);

  display->drawLine(w - 16, 54, w - 16, h - 54, cyanVeryDim);
  display->drawLine(w - 21, 68, w - 21, h - 68, cyanDim);

  // Subtle upper side brackets
  display->drawLine(56, 14, 98, 14, cyanVeryDim);
  display->drawLine(98, 14, 112, 24, cyanVeryDim);

  display->drawLine(w - 57, 14, w - 99, 14, cyanVeryDim);
  display->drawLine(w - 99, 14, w - 113, 24, cyanVeryDim);
}
...
void drawTronFullGrid(
  ...
  // Vertical sky grid
  for (int d = -65; d <= 65; d += 26) {
    if (d == 0) {
      continue;
    }

    int x1 = cx + nx * (d * 1.5f) - ux * 260;
    int y1 = cy + ny * (d * 1.5f) - uy * 260 - pitchOffset;

    int x2 = cx + nx * (d * 1.5f) + ux * 260;
    int y2 = cy + ny * (d * 1.5f) + uy * 260 - pitchOffset;
...
void drawTronFullAircraftCue(
  ...
  // Glow
  drawTronFullThickLine(display, cx - 72, wingY, cx - 22, wingY, 5, cyanDim);
  drawTronFullThickLine(display, cx + 22, wingY, cx + 72, wingY, 5, cyanDim);

  // Angular symbol
  display->drawLine(cx - 72, wingY, cx - 42, wingY, cyanBright);
  display->drawLine(cx - 42, wingY, cx - 29, wingY - 9, cyanBright);
  display->drawLine(cx - 29, wingY - 9, cx - 12, wingY - 9, cyanBright);
  display->drawLine(cx - 12, wingY - 9, cx, wingY - 18, cyanBright);
  ...
  display->drawLine(cx - 22, wingY + 6, cx - 8, wingY + 15, cyanBright);
  display->drawLine(cx + 22, wingY + 6, cx + 8, wingY + 15, cyanBright);
  display->drawLine(cx - 8, wingY + 15, cx + 8, wingY + 15, cyanBright);
...
void drawTronFullBottomDecor(
  ...
  display->drawLine(56, h - 25, w - 57, h - 25, cyanVeryDim);
  display->drawLine(78, h - 19, w - 79, h - 19, cyanDim);
  display->drawLine(104, h - 13, w - 105, h - 13, cyanBright);

  // Center lower notch
  int cx = w / 2;

  display->drawLine(cx - 36, h - 29, cx - 16, h - 8, cyanDim);
  display->drawLine(cx + 36, h - 29, cx + 16, h - 8, cyanDim);
}
