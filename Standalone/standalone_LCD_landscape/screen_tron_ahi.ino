// ----------------------------------------------------
// Futuristic Tron-style landscape AHI
// ----------------------------------------------------
//
...
void drawTronAhiScreen(Arduino_Canvas *display, float roll, float pitch)
{
  ...
  // Landscape tuning for 280 x 240.
  //
  // The sphere is wider/larger than portrait but leaves room for
  // the top ATTITUDE title and lower decorative frame.
  ...
  // Clip escaping pitch/grid elements
  ...
  // Bottom decorative circuit lines only; no unavailable data shown.
  display->drawLine(74, 224, 206, 224, cyanVeryDim);
  display->drawLine(86, 230, 194, 230, cyanDim);
  display->drawLine(102, 236, 178, 236, cyanVeryDim);
}
...
void drawTronOuterFrame(
  ...
  display->drawLine(14, 52, 14, h - 52, cyanDim);
  display->drawLine(18, 66, 18, h - 66, cyanBright);

  display->drawLine(w - 15, 52, w - 15, h - 52, cyanDim);
  display->drawLine(w - 19, 66, w - 19, h - 66, cyanBright);

  // Small circuit dots
  for (int y = 40; y <= h - 40; y += 18) {
    display->drawPixel(9, y, cyanDim);
    display->drawPixel(w - 10, y, cyanDim);
  }

  // Top decorative side brackets
  display->drawLine(42, 18, 92, 18, cyanDim);
  display->drawLine(92, 18, 104, 28, cyanDim);

  display->drawLine(w - 43, 18, w - 93, 18, cyanDim);
  display->drawLine(w - 93, 18, w - 105, 28, cyanDim);

  // Lower decorative angular panels
  display->drawLine(28, h - 24, 92, h - 24, cyanDim);
  display->drawLine(92, h - 24, 104, h - 14, cyanDim);
  display->drawLine(104, h - 14, 176, h - 14, cyanDim);
  display->drawLine(176, h - 14, 188, h - 24, cyanDim);
  display->drawLine(188, h - 24, w - 28, h - 24, cyanDim);
}
...
void drawTronTitle(
  ...
  // Angular title plate
  display->fillRect(cx - 58, 7, 116, 24, panelBlack);

  display->drawLine(cx - 58, 19, cx - 46, 7, cyanDim);
  display->drawLine(cx - 46, 7, cx + 46, 7, cyanBright);
  display->drawLine(cx + 46, 7, cx + 58, 19, cyanDim);
  display->drawLine(cx + 58, 19, cx + 46, 31, cyanDim);
  display->drawLine(cx + 46, 31, cx - 46, 31, cyanBright);
  display->drawLine(cx - 46, 31, cx - 58, 19, cyanDim);

  display->setTextSize(2);
  display->setTextColor(cyanBright);
  display->setCursor(cx - 48, 12);
  display->print("ATTITUDE");

  // Small chevrons
  display->fillTriangle(cx - 68, 17, cx - 62, 13, cx - 62, 21, cyanBright);
  display->fillTriangle(cx + 68, 17, cx + 62, 13, cx + 62, 21, cyanBright);
}
...
void clearTronOutsideCircle(
  ...
  int topY = cy - radius;
  int bottomY = cy + radius + 1;

  if (topY > 34) {
    display->fillRect(0, 34, screenW, topY - 34, backgroundColor);
  }

  if (bottomY < screenH) {
    display->fillRect(0, bottomY, screenW, screenH - bottomY, backgroundColor);
  }
...
void drawTronAircraftCue(
  ...
  // Outer glow
  drawTronThickLine(display, cx - 62, wingY, cx - 20, wingY, 5, cyanDim);
  drawTronThickLine(display, cx + 20, wingY, cx + 62, wingY, 5, cyanDim);

  // Angular wing shape
  display->drawLine(cx - 62, wingY, cx - 36, wingY, cyanBright);
  display->drawLine(cx - 36, wingY, cx - 25, wingY - 9, cyanBright);
  display->drawLine(cx - 25, wingY - 9, cx - 11, wingY - 9, cyanBright);
  display->drawLine(cx - 11, wingY - 9, cx, wingY - 18, cyanBright);
